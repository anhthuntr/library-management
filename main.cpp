	...
	while (selection!=7) {
		...
		switch (selection) {
			case 1: 
				balance=0;
				numbooks=0;
				cout<<"Enter new patron ID: "; cin>>ID;
				cin.ignore();
			...
			case 4:
				cout<<"Enter patron ID wanted to remove: "; cin>>ID;
				pat.delpat(ID); break;
			case 5:
...
void bookmenu() {
	...
	while (selection!=6) {
		...
		switch (selection) {
			case 1:
				cout<<"Enter new book ID: "; cin>>libID;
				cin.ignore();
				cout<<"Enter title: "; getline(cin,title);
				cout<<"Enter author's name: "; getline(cin,author);
				cout<<"Enter ISBN: "; cin>>isbn;
				cout<<"Enter price: "; cin>>price;
				cout<<"Enter status (available, in, out, lost, repair): "; cin>>status;

				bo.addBook(title, author, isbn, libID, price, status);
				break;
			case 2:
				cout<<"Enter book ID wanted to edit: "; cin>>ID;
				bo.editBook(ID); break;
			case 3:
				cout<<"Enter book ID wanted to find: "; cin>>ID;
				bo.findBook(ID); break;
			case 4: 
				cout<<"Enter book ID wanted to delete: "; cin>>ID;
				bo.delBook(ID); break;
			case 5:
				bo.printList(); break;
		}
	...
	while (selection!=8 && !flag) {
		...
		switch(selection) {
			case 1: 
				...
				//each transaction will have its unique key
				//thus loanid cannot be same as ones already existed
				if (trans.checkid(loanid)==0) break;
				...
				if (pat.check(patid)>=6) //total books borrowed cannot exceed 6
					cout<<"Exceed maximum number of books can borrow.\n";
				else if (bo.check(bookid)!= " available") //book has to be available
					cout<<"Book not availalbe.\n";
				else if (trans.checkStt(patid)=="overdue") //patron has no overdue book
					cout<<"Cannot issue book due to currently having overdue book.\n";

				if (pat.check(patid)<6 && bo.check(bookid)==" available" 
					&& trans.checkStt(patid)=="normal") {
					//format date
					ss<<setw(2)<<setfill('0')<<date.getCurrentMonthAsString()<<"/"<<setw(2)
					  <<setfill('0')<<date.getCurrentDayAsString()<<"/"
					  <<date.getCurrentYearAsString();

					strDate=ss.str(); //extract date

					/* add 10 days to due date */
					time_t t;
					struct tm *tmp;
					time(&t);
					...
					strDue=oss.str(); //extract due

					trans.addloan(loanid, patid, bookid, amt, strDate, strDue, stt, recheck);

					//get time when checking out book from system clock
			...
			case 2:
				...
				//a loop to get date
				do {
					cout<<"Enter check-in date (mm/dd/yyyy): "; cin>>strDate;
            	...
            	trans.delloan(loanid);
            	break;
            case 3:
            	cout<<"Enter transaction ID; "; cin>>ID;
            	trans.findloan(ID); break;
            case 4:
            	cout<<"Enter book ID to recheck: "; cin>>ID;
				/* add 10 days to due date */
				time_t t;
				struct tm *tmp;
				time(&t);
				...
				newDue=iss.str();
				trans.recheck(ID,newDue); //set new duedate
				cout<<"Due date updated.\n";
				break;
			case 5:
				trans.getoverdue("overdue"); break;
			case 6:
				trans.printList(); break;
			case 7:
				cout<<"Enter patron ID: "; cin>>patid;
				trans.getbook(patid);
				break;
